program mpicuda
	...
	integer(kind=cuda_stream_kind), dimension(:), allocatable ::g_stream
	!integer(kind=cuda_stream_kind)::g_stream
	...
	do GPU = 1, num_gpus
		...
		gpu_istat = cudaStreamCreate(g_stream(1))
		!gpu_istat = cudaStreamCreate(g_stream)
	end do
	...
	do k = 1, ntimes
		...
		do GPU = 1, num_gpus
			do i = 1, n
				h_a(GPU,i) = i*1D0!sin(i*1D0)**2
				h_b(GPU,i) = i*1D0!cos(i*1D0)**2
		...
		do GPU = 1, num_gpus
			gpu_istat=cudaSetDevice(gpu_devices(GPU))
			write(*,*) 'cudaMemcpyAsync'
			gpu_istat = cudaMemcpyAsync(d_a(:), h_a(GPU,:), n, cudaMemcpyHostToDevice, g_stream(1))
			gpu_istat = cudaMemcpyAsync(d_b(:), h_b(GPU,:), n, cudaMemcpyHostToDevice, g_stream(1))
			write(*,*) 'cudaStreamSynchronize'
			!gpu_istat = cudaStreamSynchronize(g_stream(1))
		end do
		call deviceSynchronize(gpu_devices)
		...
		do GPU = 1, num_gpus
			gpu_istat=cudaSetDevice(gpu_devices(GPU))
			call vecAdd_kernel<<<gridSize, blockSize, 0, g_stream(1)>>>(n, d_a, d_b, d_c)
		end do
		call deviceSynchronize(gpu_devices)
		
		do GPU = 1, num_gpus
			gpu_istat=cudaSetDevice(gpu_devices(GPU))
			gpu_istat = cudaMemcpyAsync(h_c(GPU, :), d_c(:), n, cudaMemcpyDeviceToHost, g_stream(1))
		end do
		call deviceSynchronize(gpu_devices)
	...
	!do GPU = 1, num_gpus
	!	gpu_istat = cudaSetDevice(gpu_devices(GPU))
		gpu_istat = cudaStreamDestroy(g_stream(1))
		!gpu_istat = cudaStreamDestroy(g_stream)
	!end do
	!end do
